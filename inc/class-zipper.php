...
class Zipper {
	...
	private $tmp     = 'tmp_file.zip';

	function __construct( $slug ) {
		$zip = new \ZipArchive();

		if ( ! copy( $this->genesis, $this->tmp ) ) {
			\WP_CLI::error( ' Failed to copy temp zip file', true );
		}
		...
		if ( $result ) {
			...
			if ( $rename ) {
				...
				\WP_CLI::error( 'Failed to rename temp directory' );
			}
		} else {
			\WP_CLI::error( 'Failed to open temp zip file', true );
		}
